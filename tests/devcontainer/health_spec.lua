...
local function stub_health()
	...
	vim.health = {
		start = function(s)
			table.insert(captured.start, s)
		end,
		ok = function(s)
			table.insert(captured.ok, s)
		end,
		warn = function(s, _adv)
			table.insert(captured.warn, s)
		end,
		error = function(s, _adv)
			table.insert(captured.error, s)
		end,
		info = function(s)
			table.insert(captured.info, s)
		end,
	}
	captured.restore = function()
		vim.health = original
...
describe("devcontainer.health.check", function()
	...
	it("warns when --version times out", function()
		...
		vim.system = function(argv, _opts, _on_done)
			if argv[1] == "devcontainer" then
				return {
					wait = function()
						return { code = 0, signal = 9, stdout = "", stderr = "" }
					end,
					kill = function() end,
				}
			end
	...
	it("errors when devcontainer.json is missing", function()
		...
		require("devcontainer").config =
			{ cli = "devcontainer", workspace_folder = "/tmp/no-such-dir-xyz", auto_attach = true }
		package.loaded["devcontainer.config"] = nil
		local h = stub_health()
		require("devcontainer.health").check()
		h.restore()
