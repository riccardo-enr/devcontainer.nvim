...
local function parse_outcome(stdout)
  ...
  if not last_obj then return nil end
...
local function notify(msg, level)
  vim.schedule(function() vim.notify("devcontainer.nvim: " .. msg, level) end)
end
...
local function diagnose(stderr, outcome)
  if outcome and outcome.message then return tostring(outcome.message) end
  local s = stderr or ""
...
local function build_argv(rebuild)
  ...
  local ws = (require("devcontainer.config")).workspace_folder()
...
function M.up(opts)
  ...
  local function fire(state, err)
    ...
    fired = true
    vim.schedule(function() on_exit(state, err) end)
  end
  ...
  local ok, _ = pcall(vim.system, argv, {
    ...
    if res and res.stdout and stdout == "" then stdout = res.stdout end
    if res and res.stderr and stderr == "" then stderr = res.stderr end
...
function M.status(opts)
  ...
  if M.state.container_id then
    vim.schedule(function() on_exit(M.state, nil) end)
    return
  end
